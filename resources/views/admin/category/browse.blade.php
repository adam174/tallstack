...
<div class="flex justify-between mb-9">
   <h3 class="text-3xl font-medium text-gray-700">Les Categories</h3>
    <a class="inline-block px-6 py-2 text-xs font-medium leading-6 text-center text-green-500 uppercase transition bg-transparent border-2 border-green-500 rounded ripple hover:bg-green-100 focus:outline-none"
        href="{{route('voyager.categories.create')}}">
            Créer
          </a>
</div>
<div>
      <table class="min-w-full table-auto">
        ...
        <tbody class="bg-gray-200 ">
          ...
          <tr class="bg-white border-4 border-gray-200">
            ...
            <td class="px-3 py-2 text-center">
              <button type="button"
                  x-on:click="window.location='{{ url("admin/categories/".$category->id) }}'" class="px-4 py-2 text-white bg-indigo-700 border rounded-md hover:bg-white hover:border-indigo-500 hover:text-black ">
                Details
              </button>
            </td>
            <td class="px-3 py-2 text-center">
                <form action="{{route('voyager.categories.destroy',$category->id)}}" id="delete_form" method="POST">
                        {{ method_field('DELETE') }}
                        {{ csrf_field() }}
